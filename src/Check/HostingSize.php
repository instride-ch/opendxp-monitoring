...
{
    protected const string IDENTIFIER = 'device:hosting_size';
    ...
    public function check(): ResultInterface
    {
        ...
        if ($this->criticalThreshold > 0 && $size >= $this->criticalThreshold) {
            return new Failure(
                \sprintf('Hosting size is too high: %s', FileSystemHelper::formatBytes($size)),
                $data
            );
        }
        ...
        if ($this->warningThreshold > 0 && $size >= $this->warningThreshold) {
            return new Warning(
                \sprintf('Hosting size is high: %s', FileSystemHelper::formatBytes($size)),
                $data
            );
        }

        return new Success(
            \sprintf('Hosting size is %s', FileSystemHelper::formatBytes($size)),
            $data
        );
    }
