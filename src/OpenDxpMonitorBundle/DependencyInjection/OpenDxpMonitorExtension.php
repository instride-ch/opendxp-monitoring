...
namespace Instride\Bundle\OpenDxpMonitorBundle\DependencyInjection;
...
class OpenDxpMonitorExtension extends Extension
{
    /**
     * {@inheritdoc}
     *
    ...
    public function load(array $configs, ContainerBuilder $container): void
    {
        ...
        foreach ($config['report'] ?? [] as $confName => $confValue) {
            $container->setParameter(
                \sprintf('opendxp_monitor.report.%s', $confName),
                $confValue
            );
        }
        ...
        foreach ($config['checks'] ?? [] as $checkName => $checkConfig) {
            foreach ($checkConfig as $confName => $confValue) {
                $container->setParameter(
                    \sprintf('opendxp_monitor.checks.%s.%s', $checkName, $confName),
                    $confValue
                );
            }
        }
