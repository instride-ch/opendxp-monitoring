...
{
    protected const string IDENTIFIER = 'device:database_table_size';
    ...
    public function check(): ResultInterface
    {
        ...
        foreach ($sizes as $size) {
            if ($this->criticalThreshold > 0 && $size['size'] >= $this->criticalThreshold) {
                $data['critical'][$size['table']] = FileSystemHelper::formatBytes($size['size']);
                continue;
            }

            if ($this->warningThreshold > 0 && $size['size'] >= $this->warningThreshold) {
                $data['warning'][$size['table']] = FileSystemHelper::formatBytes($size['size']);
                continue;
            }
