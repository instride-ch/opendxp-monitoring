...
task('opendxp:monitor:health-report', static function () {
    run('cd {{release_path}} && {{bin/console}} opendxp:monitor:health-report');
})->select('disable!=monitor_health_report');

task('opendxp:monitor', [
    'opendxp:monitor:health-report',
]);

after('deploy:success', 'opendxp:monitor');
